...
编辑操作：撤销 / 重做、向下复制（支持多选）、从此运行（跳过前面动作直接试运行）、保存后清除撤销历史。分辨率样式可在左侧“执行设置 → 分辨率样式 → 设置…”中维护；动作会保存选择时的宽高、刷新率和缩放比例快照（缩放默认 100%）。参照窗口可以留空：留空时直接改当前软件所在的显示器（不需要游戏窗口打开），选了窗口则优先改该窗口所在的显示器；该窗口没打开时自动退回软件所在显示器并记录一条提示，不会中断工作流。
