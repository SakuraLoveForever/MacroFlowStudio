...
网格逐行扫描适合处理“列表中每一行都按左右条件判断，再点击指定列”的界面：先选择一张全屏图片，用水平线和垂直线划分单元格，图片只用于确定网格位置；再分别设置左侧、右侧的识别条件以及需要点击的列。脚本实际执行到该动作时，会重新截取当前屏幕，按保存的单元格位置逐行扫描当前画面，左右条件同时满足后才点击该行的指定单元格。

![网格逐行扫描的全屏网格底图](docs/screenshots/grid-row-scan-fullscreen.png)

点击动作不会改变网格划分。测试识别用于复核已保存的网格配置：使用事先选择的底图展示单元格和 OCR 结果，不会重新截图，也不会执行点击；正式脚本执行则始终识别执行时截取的当前屏幕。

![网格逐行扫描的识别结果](docs/screenshots/grid-row-scan-recognition-result.png)

结果窗口会铺满屏幕显示所选底图，并在待识别单元格上标出边框和 OCR 文字；点击单元格不叠加识别结果。顶部工具条可以拖动，支持切换原图 / 识别结果，图片也可以使用鼠标滚轮放大或缩小。
