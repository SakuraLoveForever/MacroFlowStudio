...
用户反馈：右侧没有空白问题，真正的问题是**工具栏太挤、按钮太多、不够小巧**。
因此这一轮按「工具栏收敛」为主，配套下调窗口尺寸。

### 工具栏：19 个动作按钮 → 一行

| 项目 | 改造前 | 改造后 |
| --- | --- | --- |
| 脚本页动作按钮 | 19 个全部常驻，占两行 | 常驻 6 个 + 「+ 添加动作 ▾」菜单 13 项 |
| 插入位置 | 独立分组占第二行 | 折进「⋯ 更多」菜单 |
| 脚本插入 / 目录设置 / 模块管理 | 分散在文件行与编辑组 | 也在「⋯ 更多」里（文件行原入口保留） |
| 工具栏内边距 | 16/4/16/10 | 12/3/12/8 |
| `toolbar_spec_rows` | 用于两行排版 | 已删除（无调用方），换成 `split_toolbar_specs` |

两个入口来自**同一份按钮清单**（`_script_action_button_specs`），执行同一个命令
函数，快捷键与右键菜单不变，所以功能一个都没少。这一点由测试守住：

- `test_compact_toolbar_keeps_every_action_reachable`：常驻 ∪ 菜单 = 完整清单，
  无重复入口，每个命令名都能在主窗口上解析到。
- `test_add_action_menu_lists_every_overflow_action`：菜单项恰好覆盖全部非常驻动作。
- `test_script_toolbar_stays_on_one_row_with_an_overflow_menu`：源码里不再出现
  两行排版与「添加动作」标题。
...
| 常量 | 改造前 | 改造后 |
| --- | --- | --- |
| `MIN_MAIN_WIDTH` | 1180 | 1024 |
| `MIN_MAIN_HEIGHT` | 640 | 600 |
| 默认窗口 | min(1440, 屏宽-80) × min(820, 屏高-80) | min(1100, 屏宽-80) × min(700, 屏高-80) |

新增 `TARGET_MAIN_WIDTH/HEIGHT`，并用 `tests/test_ui_contract.py` 固化「目标尺寸、
最小尺寸、小屏不溢出」三条约束（都不创建窗口）。

### 仍未做

- 侧栏 372 → 240–260 逻辑像素：侧栏里有 `wraplength=300` 的长说明文字与多个
  并排控件，需要先把说明搬进帮助徽章/设置对话框才收得下去，属于阶段 3 的
  设置对话框那一项，尚未动手。
